...
/** Allowed candidate counts in the Ranking comparator. */
export const RANKING_COUNTS = [2, 3, 4, 5] as const
export type RankingCount = (typeof RANKING_COUNTS)[number]
 ...
 */
export function applyRankingPick(
  medias: Media[],
  picked: number,
 ...
 * any random sample is allowed, including overlap with the prior round.
 */
export function nextRankingCandidates(
  mediaLength: number,
  count: number,
  ...
  if (count < 2 || count > 5) {
    throw new Error('nextRankingCandidates: count must be between 2 and 5')
  }
  if (mediaLength < count) {
    throw new Error(
      'nextRankingCandidates: not enough media entries for this count',
    )
  }
...
export function effectiveRankingCount(
  preferred: RankingCount,
  mediaLength: number,
): number {
